 ...
 * exposes [maybeShow] which respects a minimum interval between
 * impressions so we don't spam the user.
 *
...
class AdsManager(context: Context) {
    ...
    private var quizSessionsSinceLastAd: Int = 0
    ...
    /**
     * Called after a quiz session ends. Shows the interstitial if:
     *  - an ad is currently loaded,
     *  - at least [MIN_SESSIONS_BETWEEN_ADS] quiz sessions have passed,
     *  - at least [MIN_INTERVAL_MS] elapsed since the previous impression.
     * Always preloads the next creative when done.
     */
    fun maybeShow(activity: Activity) {
        quizSessionsSinceLastAd++
        val now = System.currentTimeMillis()
        val enoughSessions = quizSessionsSinceLastAd >= MIN_SESSIONS_BETWEEN_ADS
        val enoughTime = now - lastShownAtMs >= MIN_INTERVAL_MS
        val ad = loadedAd
        if (ad != null && enoughSessions && enoughTime) {
            lastShownAtMs = now
            quizSessionsSinceLastAd = 0
            ad.show(activity)
    ...
    companion object {
        ...
        private const val MIN_SESSIONS_BETWEEN_ADS = 2
    }
}
